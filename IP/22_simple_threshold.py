...
_, th1 = cv2.threshold(img, 50, 255, cv2.THRESH_BINARY)#Here less than 50 shows black color and 50 to 255 shows white
_, th2 = cv2.threshold(img, 50, 255, cv2.THRESH_BINARY_INV)
_, th3 = cv2.threshold(img, 127, 255, cv2.THRESH_TRUNC)#here less than 127 same and above will uniformly distribute
_, th4 = cv2.threshold(img, 127, 255, cv2.THRESH_TOZERO)#here less than 127 black and other same
_, th5 = cv2.threshold(img, 127, 255, cv2.THRESH_TOZERO_INV)#here less than 127 same and other black
