...
img = cv2.imread('color_balls.jpg',0)
_,mask= cv2.threshold(img,230,255,cv2.THRESH_BINARY_INV)#bg must be white
...
kernel = np.ones((3,3),np.uint8)# 5x5 kernel with full of ones.Kernel pass throufh ever pixel  
d = cv2.dilate(mask,kernel) #iterations = 2 (optional parameters) iterations = 2
cv2.imshow("dilate==",d)


#if we want then plot it
