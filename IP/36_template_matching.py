...
template = cv2.imread("part.png", 0)
w, h = template.shape[::-1]#revert w and h with h and w by slice

#this function accept prameters (img,template,method)
res = cv2.matchTemplate(grey_img, template, cv2.TM_CCORR_NORMED)
print("res==",res)
threshold = 0.999#usually res is under .85 so pass it according to that(near to it)
...
for i in zip(*loc[::-1]):
    ...
    count+=1#we only want 1 rectangle to be detected so we want count to be 1
...
"""
img = cv2.imread('avengers1.png',0)
img2 = img.copy()
template = cv2.imread('part.png',0)
