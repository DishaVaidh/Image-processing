...
import cv2
import numpy as np
img = cv2.imread("color_ball2.jpg")
img = cv2.resize(img,(600,700))
img1 = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
ret,thresh = cv2.threshold(img1,127,255,0)#in thresh 0 means binary invert
...
#contour just pass -1(coz we want all countours we selected in threshold are shown here,but if we want specific contour then pass index of that contour from list
#Draw the contours
