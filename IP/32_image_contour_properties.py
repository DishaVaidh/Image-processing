...
blur = cv2.medianBlur(img1,11)#remove noise
...
for c in cnts:
    epsilon = 0.0001*cv2.arcLength(c,True)#it must be less 
    data= cv2.approxPolyDP(c,epsilon,True)#min vertex edges determined
