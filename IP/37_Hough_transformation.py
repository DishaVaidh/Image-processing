...
edges = cv2.Canny(gray,10,200,apertureSize = 3)#edge detect
...
#Second Type of Hough Transformation(more accurate)
"""
img = cv2.imread('chess.png')
img = cv2.resize(img,(400,400))
gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
...
lines = cv2.HoughLinesP(edges,1,np.pi/180,100,minLineLength=8,
                        maxLineGap=100)#last 2 arguments extra and change according to image
for line in lines:
    x1,y1,x2,y2 = line[0]
